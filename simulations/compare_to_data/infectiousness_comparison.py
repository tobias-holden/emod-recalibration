...
def prepare_infectiousness_comparison_single_site(sim_df, site):
    ...
    ref_df = ref_df[["freq_frac_infect", "agebin", "densitybin", "fraction_infected_bin", "Site", "month", "site_month", "year",
                     "num_in_group", "count"]]
    ...
    for a in age_bins:
        for f in frac_infected_bins:
            ...
            for m,sdf in subplot_df.groupby("month"):
                ...
                plt.plot(sdf["densitybin"], sdf["reference"], label=f"{m}", c=f"C{c}", marker='o')
                plt.plot(sdf["densitybin"], sdf["simulation"], label=f"{m}", c=f"C{c}", linestyle='dashed')
                plt.xscale("log")
                plt.title(label=f"Age: {a} Inf_Bin: {f}")
                c += 1
                plt.ylim([0,1.02])
                plt.xlim([0.4,5000])
