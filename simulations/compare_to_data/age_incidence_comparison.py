...
sites = load_sites()
for site in sites:
    if coord_csv.at[site, 'age_incidence'] == 1 :
        incidence_sites.append(site)

print(f"Incidence Sites: {incidence_sites}")
coord_csv = pd.read_csv(manifest.simulation_coordinator_path)

def prepare_incidence_comparison_single_site(sim_df, site):
    """
    ...
    sim_df = sim_df.groupby(["param_set", "Age", "Site"]).agg({"Incidence": "mean", "Population": "mean"}).reset_index()
    ...
    filepath_ref = os.path.join(manifest.base_reference_filepath,
                                coord_csv[coord_csv['site'] == site]['age_incidence_ref'].iloc[0])
    ...
    ref_df['ref.Trials'] = ref_df['POP']
    ref_df['ref.Observations'] = ref_df['POP']*ref_df['Incidence']

    # Prepare merge
    ref_df.rename(columns={"Incidence": "reference",
                           "POP": "ref_pop_size",
                           "START_YEAR": "ref_year"},
                  inplace=True)
    ref_df = ref_df[["reference", "mean_age", "Site", "ref_pop_size", "ref_year", "ref.Trials","ref.Observations"]]
    
    sim_df.rename(columns={"Incidence": "simulation"},
                  inplace=True)
    ...
    #Need to make sure sim incidence cannot be exactly 0, otherwise resulting likelihoods can be infinite/NaN
...
def compute_incidence_likelihood2(combined_df):
    df=combined_df
...
def compute_inc_LL_by_site(site, numOf_param_sets):
    ...
    ll_by_param_set["metric"] = "incidence"

    if len(missing_param_sets) > 0:
        print(f'Warning {site} is missing param_sets {missing_param_sets} for incedence')
    
    return ll_by_param_set


def compute_inc_LL_for_all_sites(numOf_param_sets):
    df_by_site = []
    for s in incidence_sites:
        df_this_site = compute_inc_LL_by_site(s, numOf_param_sets)
...
def plot_incidence_comparison_all_sites(param_sets_to_plot=None,plt_dir=os.path.join(manifest.simulation_output_filepath, "_plots")):
    for s in incidence_sites:
        plot_incidence_comparison_single_site(s, param_sets_to_plot=param_sets_to_plot,plt_dir=plt_dir)


if __name__=="__main__":
    #plot_incidence_comparison_all_sites()
    print(compute_inc_LL_for_all_sites())
