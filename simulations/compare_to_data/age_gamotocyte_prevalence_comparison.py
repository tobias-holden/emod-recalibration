...
gametocyte_prevalence_sites = []
sites = load_sites()
for site in sites:
    if coord_csv.at[site, 'age_gametocyte_prevalence'] == 1 :
        gametocyte_prevalence_sites.append(site)

print(f"Gametocyte Prevalence vs Age Sites: {gametocyte_prevalence_sites}")
coord_csv = pd.read_csv(manifest.simulation_coordinator_path)


def prepare_gametocyte_prevalence_comparison_single_site(sim_df, site):
    """
    ...
    sim_df.rename(columns={'Gametocyte prevalence': 'prevalence'}, inplace=True)
    ...
    # if len(np.unique(sim_df['year'])) > 1:
    #     og = np.min(sim_df['year'])
    #     sim_df['month'] = sim_df['month'] + 12 * (sim_df['year']-og) 
...
def compute_gametocyte_prevalence_likelihood(combined_df):
    """
...
def compute_gametocyte_prev_LL_by_site(site, numOf_param_sets):
...
def compute_gametocyte_prev_LL_for_all_sites(numOf_param_sets):
    df_by_site = []
    for s in gametocyte_prevalence_sites:
        df_this_site = compute_gametocyte_prev_LL_by_site(s,numOf_param_sets)
        df_by_site.append(df_this_site)

    return pd.concat(df_by_site)


def plot_gametocyte_prevalence_comparison_single_site(site, param_sets_to_plot=None,plt_dir=os.path.join(manifest.simulation_output_filepath, "_plots")):
    ...
    plt.xlabel("Age")
    plt.ylabel("Gametocyte Prevalence")
    plt.title(site)
    plt.legend()
    plt.tight_layout()
...
def plot_gametocyte_prevalence_comparison_all_sites(param_sets_to_plot=None,plt_dir=os.path.join(manifest.simulation_output_filepath, "_plots")):
    for s in gametocyte_prevalence_sites:
        plot_gametocyte_prevalence_comparison_single_site(s, param_sets_to_plot=param_sets_to_plot,plt_dir=plt_dir)
